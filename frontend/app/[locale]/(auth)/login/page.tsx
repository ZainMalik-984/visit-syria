...
export default function LoginPage() {
  const t = useTranslations('login'); // Namespace to 'login' in JSON
      ...
      <div className="w-full max-w-lg space-y-8 rounded-2xl bg-white/80 dark:bg-zinc-800/75 backdrop-blur shadow-2xl ring-1 ring-zinc-200/60 dark:ring-zinc-700/60 p-8 animate-fadeIn">
        ...
        {phase === 'login' && (
          <form onSubmit={handleLogin} className="space-y-6">
            <h1 className="text-center text-3xl font-extrabold text-indigo-600 dark:text-indigo-400">{t('title')}</h1>
            ...
            <Button loading={loading}>{t('loginButton')}</Button>
        ...
        {phase === 'verify-otp' && (
          <form onSubmit={handleVerify} className="space-y-6">
            <h2 className="text-center text-2xl font-bold text-indigo-600 dark:text-indigo-400">{t('verifyTitle')}</h2>
            <p className="text-center text-sm text-zinc-600 dark:text-zinc-400">
              {t('verifyDescription', { email })} {/* Dynamic interpolation */}
            </p>
            ...
            <Button loading={loading} disabled={otp.length !== 6}>{t('verifyButton')}</Button>

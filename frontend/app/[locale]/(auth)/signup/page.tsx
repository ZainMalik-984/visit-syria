...
export default function SignUp() {
  const t = useTranslations('signup'); // Namespace to 'signup' in JSON
              ...
              {(['customer', 'supplier'] as Role[]).map(r => (
                ...
                >
                  <input
                    ...
                    className="sr-only"
                  />
                  {t(`${r}Role`)} {/* Dynamic key: 'customerRole' or 'supplierRole' */}
                </label>
              ))}
            </div>

            <Button loading={loading}>{t('signUpButton')}</Button>
          </form>
        )}
          ...
          <form onSubmit={handleVerify} className="space-y-6">
            <div className="text-center space-y-1">
              ...
              <p className="text-sm text-zinc-600 dark:text-zinc-400">
                {t('verifyDescription', { email: form.email })} {/* Dynamic interpolation */}
              </p>
            </div>
